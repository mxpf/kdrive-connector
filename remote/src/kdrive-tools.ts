...
import { prepareConfirmation, requireConfirmation, validateName, type SensitiveAction } from "../../src/safety.js";
...
export function registerKDriveTools(server: McpServer, client: KDriveClient, config: RuntimeConfig): void {
	server.registerTool(
		"kdrive_connection_status",
		{
			title: "Check kDrive connection",
			description: "Verify the authenticated remote connection and return the selected kDrive metadata.",
			inputSchema: {},
			annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
		},
		async () => tool(async () => ({
			connected: true,
			authentication: "OAuth-protected Cloudflare Worker with server-side Infomaniak token",
			authorizedGitHubLogin: config.authorizedGitHubLogin,
			drive: await client.getDrive(config.driveId),
		})),
	);
	...
	server.registerTool(
		...
		async ({ fileId }) => tool(() => client.getFile(config.driveId, fileId)),
	);
	...
	server.registerTool(
		...
		async ({ directoryId, cursor, limit }) => tool(() => client.listDirectory(config.driveId, directoryId, { cursor, limit })),
	);
	...
	server.registerTool(
		...
		async ({ query, directoryId, queryScope, types, cursor, limit }) => tool(() =>
			client.search(config.driveId, query, { directoryId, queryScope, types, cursor, limit })),
	);
	...
	server.registerTool(
		...
		async ({ parentId, name, color }) => tool(() => client.createDirectory(config.driveId, parentId, validateName(name), color)),
	);
	...
	server.registerTool(
		...
		async ({ directoryId, fileName, content, encoding, conflict }) => tool(async () => {
			...
			return client.upload(config.driveId, {
				bytes,
				directoryId,
	...
	server.registerTool(
		...
		async ({ action, fileId, name, destinationDirectoryId }) => tool(async () => {
			const file = await client.getFile(config.driveId, fileId);
			const destination = action === "move" && destinationDirectoryId
				? await client.getFile(config.driveId, destinationDirectoryId)
				: undefined;
	...
	server.registerTool(
		...
		async ({ fileId, name, confirmation }) => tool(async () => {
			...
			return client.rename(config.driveId, fileId, safeName);
		}),
	);
	...
	server.registerTool(
		...
		async ({ fileId, destinationDirectoryId, confirmation }) => tool(async () => {
			...
			return client.move(config.driveId, fileId, destinationDirectoryId);
		}),
	);
	...
	server.registerTool(
		...
		async ({ fileId, etag, content, encoding, confirmation }) => tool(async () => {
			...
			return client.upload(config.driveId, { bytes, fileId, etag });
		}),
	);
	...
	server.registerTool(
		...
		async ({ fileId, confirmation }) => tool(async () => {
			...
			return { trashed: await client.trash(config.driveId, fileId), fileId, recoverable: true };
		}),
	);
	...
	server.registerTool(
		...
		async ({ fileId }) => tool(() => client.restore(config.driveId, fileId)),
	);
}
